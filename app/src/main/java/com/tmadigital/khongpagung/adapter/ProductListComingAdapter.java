...
public class ProductListComingAdapter extends BaseAdapter {
    ...
    @Override
    public int getCount() {
        if (ProductList2DComingManager.getInstance().getDao() == null)
            return 0;
        if (ProductList2DComingManager.getInstance().getDao().getProducts() == null)
            return 0;
        return ProductList2DComingManager.getInstance().getDao().getProducts().size();
    }

    @Override
    public Object getItem(int position) {
        return ProductList2DComingManager.getInstance().getDao().getProducts().get(position);
    }
