...
public class ProductListNewAdapter extends BaseAdapter {
    ...
    @Override
    public int getCount() {
        if (ProductList2DNewManager.getInstance().getDao() == null)
            return 0;
        if (ProductList2DNewManager.getInstance().getDao().getProducts() == null)
            return 0;
        return ProductList2DNewManager.getInstance().getDao().getProducts().size();
    }

    @Override
    public Object getItem(int position) {
        return ProductList2DNewManager.getInstance().getDao().getProducts().get(position);
    }
